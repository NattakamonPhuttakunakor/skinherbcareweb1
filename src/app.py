from flask import Flask, request, jsonify
from flask_cors import CORS
import pandas as pd
import numpy as np
from sklearn.feature_extraction.text import TfidfVectorizer
from sklearn.metrics.pairwise import cosine_similarity
from pythainlp.tokenize import word_tokenize
from pythainlp.corpus import thai_stopwords
import sys
import os

# เริ่มต้น Server
app = Flask(__name__)
CORS(app) # อนุญาตให้ Node.js เข้ามาคุยได้

# ========================================
# 🔧 ส่วนที่ 1: ตั้งค่า Synonym และ Stopwords (สูตรเด็ดของคุณ)
# ========================================
SYNONYM_MAP = {
    'ปวด': ['เจ็บ', 'แสบ', 'บวม', 'อักเสบ', 'จุกแน่น', 'ทรมาน'],
    'คัน': ['คันๆ', 'คันมาก', 'อยากเกา', 'ยุบยิบ'],
    'ผื่น': ['ผื่นแดง', 'ผื่นคัน', 'ตุ่ม', 'ตุ่มแดง', 'ปื้น', 'ลมพิษ', 'ตุ่มใส'],
    'ไข้': ['มีไข้', 'ตัวร้อน', 'เป็นไข้', 'รุมๆ'],
    'แขน': ['ต้นแขน', 'ปลายแขน', 'ข้อศอก', 'มือ'],
    'ขา': ['ต้นขา', 'น่อง', 'เท้า', 'เข่า'],
    'มาก': ['มากๆ', 'รุนแรง', 'เยอะ', 'หนัก', 'ไม่ไหว'],
}

CUSTOM_STOPWORDS = set(thai_stopwords()) | {
    "เป็น", "มี", "รู้สึก", "อาการ", "หน่อย", "มาก", "ๆ", "ค่ะ", "ครับ", 
    "คือ", "ที่", "และ", "หรือ", "ช่วย", "ด้วย", "แล้ว", "อยาก", "ต้อง",
    "นะ", "จะ", "เอง", "ได้", "ไป", "มา", "อยู่", "ให้", "บริเวณ", "แถวๆ"
}

def expand_synonyms(text):
    text = str(text).lower()
    for main_word, synonyms in SYNONYM_MAP.items():
        for syn in synonyms:
            if syn in text:
                text += f" {main_word}" 
    return text

def thai_tokenizer(text):
    if not isinstance(text, str): return []
    text = expand_synonyms(text) 
    words = word_tokenize(text, engine="newmm", keep_whitespace=False)
    return [w for w in words if w not in CUSTOM_STOPWORDS and len(w) > 1 and not w.isnumeric()]

# ========================================
# 📂 ส่วนที่ 2: โหลดข้อมูลและเตรียมสมอง (Patching Data)
# ========================================
print("⏳ กำลังโหลดข้อมูลและเทรน AI...")

df = None
# พยายามโหลดไฟล์ (รองรับทั้งชื่อเก่าและใหม่)
possible_files = ["data.xlsx - Sheet1.csv", "data.csv", "data.xlsx"]

for f in possible_files:
    if os.path.exists(f):
        try:
            if f.endswith('.csv'):
                df = pd.read_csv(f)
            else:
                df = pd.read_excel(f)
            print(f"✅ เจอไฟล์: {f}")
            break
        except Exception as e:
            print(f"⚠️ อ่านไฟล์ {f} ไม่ได้: {e}")

if df is None:
    print("❌ Error: หาไฟล์ข้อมูลไม่เจอเลย (data.xlsx หรือ .csv)")
    sys.exit()

# ลบช่องว่างหัวตาราง
df.columns = df.columns.str.strip()

# ฟังก์ชันแก้ข้อมูล (Data Patching)
def clean_and_prepare_data(row):
    main = str(row.get('อาการหลัก', ''))
    sub = str(row.get('อาการรอง', ''))
    loc = str(row.get('ตำแหน่งที่พบบ่อย', ''))
    treatment = str(row.get('วิธีรักษาเบื้อต้น', ''))
    
    # แก้เรื่องไข้
    if 'ไข้' in treatment and 'ไข้' not in sub:
        sub += " มีไข้"
        
    # สร้าง Knowledge Text (สูตรเบิ้ลคำ เพื่อเพิ่มน้ำหนัก)
    knowledge_text = f"{row['รายชื่อโรค']} {main} {main} {sub} {loc} {loc}"
    return knowledge_text

# สร้างคอลัมน์ใหม่สำหรับเทรน
df['knowledge'] = df.apply(lambda x: clean_and_prepare_data(x), axis=1)

# สร้าง TF-IDF Matrix (สมอง)
vectorizer = TfidfVectorizer(
    tokenizer=thai_tokenizer,
    ngram_range=(1, 2),
    min_df=1,
    sublinear_tf=True
)

try:
    tfidf_matrix = vectorizer.fit_transform(df['knowledge'])
    print(f"✅ AI พร้อมทำงาน! (รู้จัก {len(df)} โรค)")
except Exception as e:
    print(f"❌ Error ตอนสร้างสมอง AI: {e}")
    sys.exit()

# ========================================
# 🔌 ส่วนที่ 3: เปิดช่องทางรับคำสั่ง (API Endpoint)
# ========================================
@app.route('/api/analyze', methods=['POST'])
def analyze():
    # รับข้อมูลจาก Node.js
    data = request.json
    user_input = data.get('symptoms', '').strip()
    
    print(f"📩 ได้รับอาการเข้ามา: {user_input}")

    if not user_input:
        return jsonify({"success": False, "message": "กรุณาระบุอาการ"})

    # 1. แปลงอาการเป็นตัวเลข
    user_vec = vectorizer.transform([user_input])
    
    # 2. คำนวณความเหมือน
    scores = cosine_similarity(user_vec, tfidf_matrix).flatten()
    
    # 3. หา Top 3
    top_indices = scores.argsort()[::-1][:3]
    
    results = []
    found_any = False

    for idx in top_indices:
        score = scores[idx]
        if score > 0.1: # เกณฑ์ความมั่นใจขั้นต่ำ 10%
            found_any = True
            row = df.iloc[idx]
            
            # เช็ค Warning
            warning_msg = ""
            if 'ไข้' in user_input and 'ไข้' in str(row.get('อาการรอง', '')):
                warning_msg = "(โรคนี้มักมีไข้ร่วมด้วย ตรงกับอาการของคุณ)"

            results.append({
                "disease": str(row['รายชื่อโรค']),
                "confidence": round(score * 100, 2),
                "symptoms": str(row['อาการหลัก']),
                "location": str(row['ตำแหน่งที่พบบ่อย']),
                "treatment": str(row.get('วิธีรักษาเบื้อต้น', 'แนะนำให้พบแพทย์')),
                "warning": warning_msg,
                "herbs": str(row.get('สมุนไพรที่เกี่ยวข้อง', '-')).split(',') # ถ้ามีคอลัมน์นี้
            })

    # ส่งผลลัพธ์กลับไปให้ Node.js
    if not found_any:
        return jsonify({
            "success": True,
            "found": False,
            "message": "ไม่พบโรคที่ตรงกับอาการชัดเจน"
        })

    return jsonify({
        "success": True,
        "found": True,
        "data": results
    })

if __name__ == '__main__':
    print("🚀 Python Server กำลังรันที่ Port 5001...")
    app.run(port=5001, debug=True)